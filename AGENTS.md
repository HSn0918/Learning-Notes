...
- 所有正式学习题都必须紧跟非空的参考答案，包括 `Q:`/`Q1:`、面试题、自检题、关键问题和疑问式题目标题。
- 普通任务清单、实验步骤和排障检查项不是题目；不要为了凑问答而伪造唯一答案。
- 统一使用 Obsidian 默认收起的 callout；题干始终放在折叠块外：
...
> [!question]- 参考答案（点击展开）
>
> Decode 会重复访问历史 Token 的 K/V；缓存它们可避免重复计算，但会占用显存。
```

- 答案中的空行、列表、表格、代码围栏和 Mermaid 每一行都要带 `>`。列表内题目的 callout 需要跟随列表缩进。
- Obsidian 中答案默认折叠；GitHub 不会折叠该 callout，但会降级为完整可读的引用块。这是仓库明确接受的兼容行为。
- 不再使用裸 `A:`、`答：`、`**参考答案**` 或“问题/回答”表格保存正式题目。
...
- 不新增没有折叠参考答案的正式学习题；`scripts/validate_notes.py` 会将其视为阻断错误。
