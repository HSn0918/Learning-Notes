...
> K8s 研发深挖和排障训练统一放在 `learning-plan/topics/`：kube-proxy、Cilium deep dive、CNI 排障、Volume 生命周期、CSI sidecar、CSI 排障。
...
---
### K8s-研发深挖（learning-plan）

| 专题 | 简介 |
|------|------|
| [kube-proxy 服务转发](../learning-plan/topics/networking/kube-proxy.md) | 讲解 kube-proxy watch Service/EndpointSlice 并维护 iptables/IPVS Service 数据面的职责边界 |
| [Cilium 深度解析](../learning-plan/topics/networking/cilium-deep-dive.md) | 讲解 Cilium endpoint、identity、BPF map、kube-proxy replacement 与 L7 策略协作模型 |
| [CNI 排障路径](../learning-plan/topics/networking/cni-troubleshooting.md) | 按 Pod IP、跨节点、ClusterIP、DNS、NetworkPolicy 五层定位 Kubernetes 网络问题 |
| [Volume 生命周期](../learning-plan/topics/storage/volume-lifecycle.md) | 串联 PVC、PV、StorageClass、VolumeAttachment、Attach、Stage、Publish 与回收流程 |
| [CSI Sidecar 体系](../learning-plan/topics/storage/csi-sidecars.md) | 讲解 external-provisioner、attacher、resizer、snapshotter、registrar 等 sidecar 如何把 K8s 对象转成 CSI RPC |
| [CSI 排障路径](../learning-plan/topics/storage/csi-troubleshooting.md) | 按 PVC Pending、Attach、Mount、Resize、Snapshot 分层定位存储插件问题 |

---
本目录共 **41** 篇 · 入门 10 / 进阶 21 / 深入 10；另有 `learning-plan/topics/` 6 篇研发深挖专题。
