...
如果你已经看过 `cloud-native/kubernetes/demos/device-plugin` 那个通用 fake device 示例，这里有三个有意识的差异点：

1. **资源名** `learning-notes.io/fake-device` → `fake-gpu.k8s.io/gpu`，对齐 NVIDIA `nvidia.com/gpu` 的形态。
...
```bash
cd cloud-native/kubernetes/demos/fake-gpu
go mod tidy
go build ./...
```
