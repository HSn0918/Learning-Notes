...
// --- 构建说明 ---
//
// 直接 `require k8s.io/kubernetes vX.Y.Z` 会失败：kubernetes 主仓的 go.mod 把一组
// staging 子模块（k8s.io/api、k8s.io/apiserver、k8s.io/kube-scheduler 等）写成
// 占位版本 v0.0.0，并靠主仓内部的 `replace => ./staging/src/...` 解析。
// 这些 replace 不会传递给下游，所以下游必须自己把每个 staging 模块
// replace 到真实发布版本（k8s v1.34.0 时全部用 v0.34.0）。
//
// 参考：https://github.com/kubernetes-sigs/scheduler-plugins/blob/master/go.mod
