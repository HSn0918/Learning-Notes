...
带 label `learning-notes/preferred=true` 的节点拿满分（`framework.MaxNodeScore`），
...
├── config.yaml              KubeSchedulerConfiguration v1 示例（profile: learning-notes-scheduler）
...
metadata:
  name: hello
spec:
  schedulerName: learning-notes-scheduler   # 不写则走 default-scheduler
  containers:
    - name: app
      image: nginx
```
...
kind: ClusterRoleBinding
metadata:
  name: learning-notes-scheduler-volume
roleRef:
  apiGroup: rbac.authorization.k8s.io
  kind: ClusterRole
...
# 我们用了独立 resourceName=learning-notes-scheduler，必须单独补一个 Role 授权这个新 lease，
# 否则 Pod 起来后 leader election 会一直 forbidden、选不出 leader。
apiVersion: rbac.authorization.k8s.io/v1
kind: Role
metadata:
  name: learning-notes-scheduler-leaderelection
  namespace: kube-system
rules:
  ...
  - apiGroups: ["coordination.k8s.io"]
    resources: ["leases"]
    resourceNames: ["learning-notes-scheduler"]
...
kind: ConfigMap
metadata:
  name: learning-notes-scheduler-config
  namespace: kube-system
data:
  config.yaml: |
    ...
    leaderElection:
      leaderElect: true
      resourceName: learning-notes-scheduler
...
metadata:
  name: learning-notes-scheduler
  namespace: kube-system
  labels: { app: learning-notes-scheduler }
spec:
  replicas: 1
  selector:
    matchLabels: { app: learning-notes-scheduler }
  template:
    metadata:
      labels: { app: learning-notes-scheduler }
    spec:
      serviceAccountName: learning-notes-scheduler
      containers:
        - name: scheduler
          image: learning-notes-scheduler:dev
...
- 业务侧只要在 Pod `spec.schedulerName` 写 `learning-notes-scheduler`，调度走向就切过来了；
...
W ... Error looking up in-cluster authentication configuration: configmaps
  ...
  "system:serviceaccount:kube-system:learning-notes-scheduler" cannot get
...
kubectl -n kube-system get lease learning-notes-scheduler
kubectl -n kube-system logs deploy/learning-notes-scheduler | grep "successfully acquired lease"

# 2. 给两个 worker 打相反的 label
kubectl label node <node-a> learning-notes/preferred=true  --overwrite
kubectl label node <node-b> learning-notes/preferred=false --overwrite
...
kubectl -n kube-system logs deploy/learning-notes-scheduler | grep "NodeLabelScore matched"
```
