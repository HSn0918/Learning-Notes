...
├── storageclass.yaml   StorageClass(provisioner=learning-notes.csi.k8s.io)
├── pvc.yaml            测试 PVC + Pod
...
```bash
# 在仓库根目录运行
cd cloud-native/kubernetes/demos/csi-hostpath
...
kubectl describe csinode <node-name>   # 看 spec.drivers[] 里是不是有 learning-notes.csi.k8s.io
```
...
# macOS 不能原生构建该 Linux mount 实现；
# 如需验证 RPC，请在 Linux 容器或 kind node 中运行二进制。
```
