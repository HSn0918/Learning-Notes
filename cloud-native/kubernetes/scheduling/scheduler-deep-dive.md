...
相关笔记：[[kubernetes-basics]] | [[scheduler-framework-source]] | [[volcano-source]] | [[k8s-interview]]
...
graph TD
    ...
    I --> J[Permit]
    J --> K{Binding Cycle}
    K --> L[PreBind / Bind / PostBind]
    L --> M[写入 apiserver]
    M --> N[Pod Scheduled]
    
    D -->|无可用节点| P[Preemption 抢占]
    P --> Q[驱逐低优先级 Pod]
    Q --> C
```
...
调度周期是串行的，绑定周期可以异步并发。选出 Node 后，调度器先调用 `Cache.AssumePod`：在进程内存中把 Pod 计入目标 Node 的已分配资源，但不写 API Server 或 etcd；随后执行 Reserve 插件和 Permit。下一轮调度因此能立即看到扣减后的资源账本，不必等待网络写入完成。

| 对比项 | Assume | Bind |
| --- | --- | --- |
| 状态位置 | Scheduler 本地 cache | API Server，最终持久化到 etcd |
| 主要目的 | 先更新资源视图，让调度主循环继续处理下一个 Pod | 持久化 `spec.nodeName` 的绑定结果 |
| 失败处理 | `Cache.ForgetPod` 回滚假定状态 | 返回错误并触发 Unreserve、回滚与重新排队 |
| 并发边界 | 位于串行 Scheduling Cycle 尾部 | 位于异步 Binding Cycle |

Bind 成功后，Scheduler 的 Informer 收到已绑定 Pod 事件，再把 assumed 状态转成已确认状态。Bind 失败时，调度器会 forget 假定 Pod，释放本地预占；Reserve 插件还必须通过 `Unreserve` 回滚自己的外部或插件状态。部分旧版本实现过 assumed Pod 的过期清理，但该机制并非稳定契约；必须以所读版本的 cache 实现为准。

Assume 解决的是单个 Scheduler 进程内的资源账本与吞吐问题，不是跨多个 Scheduler 的分布式锁。若多个调度器同时管理相同对象，最终仍需要 API Server 的并发控制和清晰的 `schedulerName` 边界。
