...
A: Multus 允许一个 Pod 同时接入多个网络（多网卡），解决了标准 CNI 只能给 Pod 分配一个网络接口的限制。典型场景是电信/5G 核心网中，控制面和数据面需要走不同网络，或者需要 SR-IOV 直通网卡获取接近裸机的网络性能。

**Q: SR-IOV 和 DPDK 有什么区别？**

A: SR-IOV 是网卡硬件虚拟化技术，Pod 直接使用物理网卡的 Virtual Function，仍然走内核协议栈但性能接近裸机；DPDK 是用户态网络驱动，完全绕过内核协议栈，适合电信数据面的高吞吐包处理场景。
