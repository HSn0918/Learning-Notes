...
1. 上传大文件如何保证不超时？
   ...
   > [!question]- 参考答案（点击展开）
   > 不让单个 HTTP 请求承载完整镜像：客户端按固定大小切片，服务端为每片独立确认，并把上传会话与合并过程解耦。入口层、应用层和对象/文件存储仍要设置与单片时长匹配的 timeout；仅调大一个全局 timeout 不能提供断点恢复。

2. 如何保证上传文件的唯一性？

   > [!question]- 参考答案（点击展开）
   > 为每次上传生成不可猜测的 `upload_id`，每个分片用 `(upload_id, part_number)` 唯一定位，并记录分片 hash/大小。最终镜像再以完整内容 digest 或业务唯一键做幂等校验；不要只依赖原始文件名。

3. 上传过程如何保证断点续传？

   > [!question]- 参考答案（点击展开）
   > 服务端持久化上传会话、总分片数和已确认分片集合，客户端重连后先查询缺片再续传。分片写入和最终合并都要幂等，完成后校验完整文件 digest；只有客户端本地记进度不够，因为服务端状态可能不同步。

4. K8s 1.27 不再内置 Docker，如何适配？

   > [!question]- 参考答案（点击展开）
   > 准确边界是 dockershim 自 Kubernetes 1.24 移除，并非 1.27 才移除。镜像仍采用 OCI/Docker Image 格式，可用 Buildah 无 daemon 构建和推送，再由节点的 containerd/CRI-O 通过 CRI 拉取运行；不需要让 kubelet 直接调用 Docker Engine。

5. 镜像仓库默认开启 HTTPS，如何配置证书或跳过验证？

   > [!question]- 参考答案（点击展开）
   > 生产环境应把仓库 CA 加入 Buildah/containers image 的 trust 配置并校验证书主机名。`--tls-verify=false` 只适合受控测试环境的临时排障，因为它会失去对中间人攻击和错误仓库地址的保护。

6. 镜像上传涉及的目录是否支持挂载 PVC？

   > [!question]- 参考答案（点击展开）
   > 上传分片和工作目录可以放 PVC，但 Buildah 的 graphroot 依赖 POSIX 文件、锁和所选 storage driver 的语义，不能直接放对象存储。应按并发写入模式选择 RWO/RWX 卷并实测 overlay/fuse-overlayfs 或 `vfs`；共享 NFS 的一致性与性能不能凭“可挂载”就视为可用。
...
> [!question]- 参考答案（点击展开）
>
> 单次流式上传整个 20G 文件，会受制于 Ingress controller、上层负载均衡（如 Nginx）的连接超时与 `client_max_body_size` 等限制，长连接极易被中断。分片上传把大文件切成多个小切片，每个切片单独发一个 HTTP 请求，单请求体积小、耗时短，不触发超时阈值；同时还能并发上传、失败重传、断点续传，整体可靠性远高于单流上传。

**Q: 分片上传如何保证文件一致性？**

> [!question]- 参考答案（点击展开）
>
> 分片本身不能自动保证合并后文件完整。方案是双层校验：前端对每个切片计算 MD5/SHA256，上传时带上摘要，后端逐片校验，不匹配则拒绝并要求重传；同时保存整个文件的 MD5，在所有切片合并完成后做一次整体校验。前端可用 spark-md5 库分块增量计算，避免一次性读入大文件导致浏览器卡死或 OOM。

**Q: 断点续传是怎么实现的？前端方案和后端方案有什么区别？**

> [!question]- 参考答案（点击展开）
>
> 核心是上传前先查询哪些切片已成功，跳过它们只传剩余部分。前端方案在本地（如 localStorage）记录已成功切片，刷新或断网后续传时跳过；缺点是换设备/清缓存就失效。后端方案把每个切片的 hash 保存在服务端，前端上传前调用「获取已上传切片信息」接口拿到已存在的 hash 列表，跨设备也能续传，更可靠，通常以后端为准。

**Q: 为什么选择 Buildah 而不是 Docker 或 nerdctl 来加载并推送镜像？**

> [!question]- 参考答案（点击展开）
>
> 关键约束是 K8s 从 1.24 起移除了内置的 dockershim，节点默认运行时收敛到 containerd/CRI-O，不再内置 Docker（本项目平台基于 K8s 1.27，已是无 Docker 环境）。Buildah 是 daemonless 工具，无需常驻 daemon，可直接以普通进程/Pod 形式在容器内运行，资源占用低，支持 OCI 和 Docker 两种镜像格式；构建和推送都能通过 `--tls-verify=false` 命令行参数直接跳过证书验证，无需改全局配置。Docker 和 nerdctl 在容器内运行还需挂载 socket 依赖外部 daemon（containerd/dockerd），不适合这种「在 Pod 里临时拉起一个镜像处理任务」的场景。

**Q: 在容器内运行 Buildah 常见会遇到哪些坑？怎么解决？**

> [!question]- 参考答案（点击展开）
>
> 一是构建/挂载失败，因为 OverlayFS 在容器内嵌套使用受限，需要以特权模式（privileged）运行并赋予网络管理、存储管理等权限，或安装 fuse-overlayfs 并在 `storage.conf` 中启用 `mount_program`；二是无法存储镜像，可把存储驱动切换到兼容性更好的 vfs（设置环境变量 `STORAGE_DRIVER=vfs`，注意笔记里写的 `BUILDAH_STORAGE=vfs` 并非真实变量名），代价是占用更多磁盘空间。

**Q: Buildah 的存储目录能挂 PVC 吗？有什么限制？**

> [!question]- 参考答案（点击展开）
>
> Buildah 关键目录是 `/var/lib/containers`（镜像与配置）和 `/var/tmp`（加载镜像的临时目录，加载完成后删除）。hostPath、emptyDir、NFS 都能正常挂载工作；但 MinIO 这类对象存储不支持 Buildah 存储镜像所需的文件语义（如硬链接、特殊文件类型、随机写），无法直接当镜像存储后端使用。

**Q: 合并、加载、推送是耗时长的异步操作，前端怎么感知进度？**

> [!question]- 参考答案（点击展开）
>
> 「合并文件并推送镜像」接口设计为异步触发，内部按合并切片 → 加载为镜像 → 推送仓库三步执行，每一步把状态写入 Redis；前端通过单独的「获取推送状态」接口轮询 Redis 中的步骤状态，从而展示当前进度而不必阻塞等待整个流程。
