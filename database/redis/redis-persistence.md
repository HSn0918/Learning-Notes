...
1. **RDB 的 fork 有什么风险？**
   ...
   > [!question]- 参考答案（点击展开）
   >
   > fork 本身会阻塞主线程（复制页表），如果 Redis 使用内存很大，fork 耗时会很长。另外 COW 在写入频繁时会导致内存翻倍。
2. **AOF 文件损坏怎么办？**

   > [!question]- 参考答案（点击展开）
   >
   > 使用 `redis-check-aof --fix` 工具修复，它会截断最后一条不完整的命令。
3. **为什么 AOF 重写不直接修改旧文件？**

   > [!question]- 参考答案（点击展开）
   >
   > 为了保证原子性，重写是生成新文件然后原子替换。如果直接修改旧文件，中途失败会导致数据不完整。
4. **everysec 策略真的最多丢 1 秒数据吗？**

   > [!question]- 参考答案（点击展开）
   >
   > 理论上是，但如果 fsync 太慢（磁盘 IO 繁忙），可能丢 2 秒的数据。
5. **Redis 7.0 的 Multi Part AOF？**

   > [!question]- 参考答案（点击展开）
   >
   > Redis 7.0 将 AOF 拆分为 base 文件（RDB/AOF 格式）+ 多个 incr 增量文件 + manifest 清单文件，解决了重写时的内存和 IO 开销。
