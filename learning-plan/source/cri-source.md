...
下面这段 ~100 行 Go 骨架展示了"启动一个最小 CRI gRPC server"的关键步骤。完整可编译版本在 `learning-plan/demos/fake-cri/`。
