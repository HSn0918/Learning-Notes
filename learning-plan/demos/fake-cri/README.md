...
一个最小可运行的 **fake CRI runtime server**，对应学习笔记 `learning-plan/cri-source.md` 第九节。
...
```bash
cd learning-plan/demos/fake-cri
go mod tidy
go build -o fake-cri .
```
...
- `learning-plan/cri-source.md` —— 主笔记 CRI 源码导读
- `learning-plan/kubelet-cri-source.md` —— kubelet 端的 syncLoop / PLEG / syncPod
- `learning-plan/cni-source.md` —— sandbox 创建后 runtime 如何调 CNI
