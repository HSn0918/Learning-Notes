...
`learning-plan/demos/kubebuilder-operator/` 下的最小 Operator 骨架，用于配合 [[controller-runtime-source]] 源码走读。**不依赖 CRD**，只监听 `ConfigMap`，所以任何集群都能跑，避免 `kubebuilder init` / `make install` 的环境摩擦。
