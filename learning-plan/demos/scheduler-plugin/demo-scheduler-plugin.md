...
func (pl *NodeLabelScore) Score(ctx context.Context, state *framework.CycleState,
    ...
    if nodeInfo.Node().Labels["learning-plan/preferred"] == "true" {
        return 100, nil
    }
    return 0, nil
}
...
leaderElection:
  leaderElect: true
  resourceName: learning-plan-scheduler        # 必须与 default-scheduler 不同！
...
3. **业务 Pod 通过 `spec.schedulerName` 路由**：写了 `learning-plan-scheduler` 才走自定义调度器，其他 Pod 走默认。
...
> 完全靠 `pod.spec.schedulerName`。这个字段默认值是 `default-scheduler`，你也可以写成 `learning-plan-scheduler`。`Scheduler.ScheduleOne` 在 `frameworkForPod` 里按这个字段选 profile，**找不到对应 profile 的 Pod 会被该调度器忽略**（不是失败，是直接不处理，留给其他调度器）。这样多个 scheduler 之间天然不会争抢同一个 Pod。
